...
/*
 * ----------------------------------------------------------------
 * TweenPlugin   (could easily be split out as a separate file/class, but included for ease of use (so that people don't need to includes another script call before loading plugins which is easy to forget)
 * ----------------------------------------------------------------
 */
...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push( function() {
	...
	_gsScope._gsDefine("plugins.CSSPlugin", ["plugins.TweenPlugin","TweenLite"], function(TweenPlugin, TweenLite) {
		 ...
		 *  - If I create a tween of elementA, that tween instance may suddenly change its target to includes 50 other elements (unintuitive if I specifically defined the target I wanted)
		...
		_split = function(element, vars, allChars, allWords, allLines) {
			...
			if (absolute) {
				if (origHeight > element.clientHeight) {
					...
					if (element.clientHeight < origHeight) { //IE8 and earlier use a different box model - we must includes padding and borders
						element.style.height = (origHeight + borderTopAndBottom)+ "px";
					}
				}
				if (origWidth > element.clientWidth) {
					element.style.width = (origWidth - padLeftAndRight) + "px";
					if (element.clientWidth < origWidth) { //IE8 and earlier use a different box model - we must includes padding and borders
